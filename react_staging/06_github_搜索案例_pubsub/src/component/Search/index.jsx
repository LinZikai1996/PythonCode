    ...
    search = () => {
        ...
        axios.get(`https://api.github.com/search/users?q=${keyWorkElement}`).then(
            response =>{
                ...
                // 发送请求成功后, 通知 list 跟新状态
                // this.props.updateAppState({isLoadingOrNo: false,users: response.data.items,})
                console.log('发送请求成功后, 通知 list 跟新状态')
                PubSub.publish('listInfo', {isLoadingOrNo: false,users: response.data.items,})
            },
            error => {
                console.log('失败', error)
                // 发送请求失败后, 通知 list 跟新状态
                // this.props.updateAppState({isLoadingOrNo: false,errorInfo: error.message,})
                console.log('发送请求失败后, 通知 list 跟新状态')
                PubSub.publish('listInfo', {isLoadingOrNo: false,errorInfo: error.message,})
            }
        )
    }
